const USER_AGENT = "GlobalNutritionApp/1.0";
...
async function searchFood() {
    ...
    } catch (e) { showError("Connection Error"); } finally { document.getElementById('loading-spinner').classList.add('hidden'); }
}
...
function renderTable(targetId, nutriments, keys, sectionId, barClass) {
    ...
    keys.forEach(k => {
        ...
        if (val !== undefined && val !== null && val !== 0) {
            ...
            tbody.innerHTML += `<tr><td><strong>${rdiObj.label}</strong></td><td>${parseFloat(val).toFixed(2)} ${rdiObj.unit}</td><td><div class="coverage-bar-container"><div class="coverage-bar ${barClass}" style="width:${perc}%"></div></div><span class="coverage-value">${perc.toFixed(0)}%</span></td><td class="norm-cell">${rdiObj.val}${rdiObj.unit}</td></tr>`;
        }
    });
    if (sectionId) document.getElementById(sectionId).classList.toggle('hidden', foundCount === 0);
}
